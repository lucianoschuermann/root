...
class JPMorganSpreadModel(MarketImpactModel):
    """
    JPMorgan model WITH explicit spread proxy.

    MI_total = ω * I * (2*PoV/(1+PoV)) + (1-ω)*I + S_c
             = I * [1 + ω*(X-1)] + S_c
    ...
    def calibrate(
        ...
        required_cols = ["Order Quantity", adv_col, vol_col, "ImpactRealised", abos_col]
        ...
        mi_total = df["ImpactRealised"].astype(float)
        spread_component = df[abos_col].astype(float) / 10000.0 / 2.0
        mi_target = mi_total - spread_component

        basic_valid = (
            (adv_eff > 0.0) & np.isfinite(adv_eff)
            & (pov > 0.0) & np.isfinite(pov)
            & (vol > 0.0) & np.isfinite(vol)
            & np.isfinite(mi_total)
            & np.isfinite(spread_component)
        ...
        if len(df) < MIN_CALIBRATION_N:
            null["calibration_note"] = "too_few_positive_spread_adjusted_targets"
            return null
    ...
    def calculate(self, out_df: pd.DataFrame, window_type: str, params: dict) -> pd.Series:
        ...
        required_cols = ["Order Quantity", adv_col, vol_col, abos_col]
        ...
        spread_component = d[abos_col].astype(float) / 10000.0 / 2.0
        ...
        spread_v = spread_component.loc[valid].to_numpy(dtype=float)

        I = alpha * (pov_v ** beta) * (vol_v ** gamma)
        MI = omega * I * (2.0 * pov_v / (1.0 + pov_v)) + (1.0 - omega) * I + spread_v
